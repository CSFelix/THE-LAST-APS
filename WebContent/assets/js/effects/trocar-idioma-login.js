...
if (idiomaAtual) {
    ...
    // tradução página login
    try { vueAtivarControladorIdiomaLogin(); }
    catch(event) {  }
    
    // tradução página cadastro
    try { vueAtivarControladorIdiomaCadastro(); }
    catch(event) { }
    
    // tradução página alterar senha
    try { vueAtivarControladorIdiomaAlterarSenha(); }
    catch(event) { }
    
    // tradução página status conta
    try { vueAtivarControladorIdiomaStatusConta(); }
    catch(event) { }
    
    // tradução página operação realizada
    try { vueAtivarControladorIdiomaOperacaoRealizada(); }
    catch(event) { }
}
